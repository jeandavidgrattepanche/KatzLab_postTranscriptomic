	...
	if args.input_file != None:
		if os.path.isfile(args.input_file) != False:
			if args.input_file.split('/')[-1] not in os.listdir('/'.join(args.input_file.split('/')[:-1])):
				...
				valid_arg += 1
			elif args.input_file.endswith('bp_Xcont.fasta') != True:
				...
				' Name and must end with ' + color.RED + '"bp_Xcont.fasta"\n\n' + color.END)
				valid_arg += 1
		else:
			print (color.BOLD+color.RED+'\nError:'+color.END+color.BOLD+' The provided Fasta file '\
			'('+color.DARKCYAN+args.input_file.split('/')[-1]+color.END+color.BOLD+')\ndoes not'\
...
def prep_folders(args):
	rRNA_folder = '../'+args.input_file.split('/')[-1].split('_200bp_Xcont.fasta')[0]+'/rRNA_Removal/'
...
def remove_rDNA(args): 	
	...
	blast_output = rRNA_folder+args.input_file.split('/')[-1].split('_200bp_Xcont.fasta')[0]+'_allSSULSUresults.tsv'
	...
	with open(rRNA_folder+args.input_file.split('/')[-1].split('_200bp_Xcont.fasta')[0]+'_rRNAseqs.fasta','w+') as HasSSU:
		for seq_rec in inFasta:
			if seq_rec.description in rDNA_Hits:
				HasSSU.write('>'+seq_rec.description+'\n'+str(seq_rec.seq)+'\n')
				with_SSULSU += 1

	with open(rRNA_folder+args.input_file.split('/')[-1].split('_200bp_Xcont.fasta')[0] + '_NorRNAseqs.fasta','w+') as NoSSU:
		for seq_rec in inFasta:
...
def clean_up(args):
	...
	home_folder = '../'+args.input_file.split('/')[-1].split('_200bp_Xcont.fasta')[0]+'/'
