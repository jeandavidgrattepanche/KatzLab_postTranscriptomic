...
def prep_folders(args):
	Home_folder_name = ('-').join(args.output_file.split('-')[0:2])
	...
	if os.path.isdir('../XlaneBleeding/') != True:
		os.system('mkdir ../XlaneBleeding/')
...
def rename_Transcriptome(args):
	...
	home_folder = '../'+('-').join(args.output_file.split('-')[0:2])+'/SizeFiltered/'
...
def clean_up(args):
	...
	os.system('mv ../'+args.output_file+'.'+str(args.minLen)+'bp.fasta ../XlaneBleeding/')
