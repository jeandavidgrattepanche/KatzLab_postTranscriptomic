...
def check_args():
	...
	if len(sys.argv[1:]) == 0:
		if len([i for i in os.listdir(os.curdir+'./') if i.endswith('bp_Xcont.fasta')]) > 0:
			infile_list += ['../'+i for i in os.listdir(os.curdir+'./') if i.endswith('bp_Xcont.fasta')]
		else:		
	...
	if args.input_file != None:
		if args.input_file.endswith('bp.fasta') == True and args.input_file.strip('../') in os.listdir(os.curdir+'./'):
			...
			' Fasta File'\
			' Name and must end with ' + color.RED + '"bp_Xcont.fasta"' + color.END)
			print (usage_msg()+'\n')
			sys.exit()
	else:
		if infile_list[0] == '':
...
def usage_msg():
	...
	' 2_Auto_rRNA_BvE.py --input_file ../Op_me_Xxma_rna.200bp_Xcont.fasta'+color.END+color.BOLD+\
...
def remove_rRNA(SizeFilt_Fasta, args):
	os.system('python3 2a_remove_rRNA.py --input_file '+SizeFilt_Fasta+' --threads '+args.threads)
...
def remove_Bact(rRNA_Free_Fasta, args):
	fasta_withBact = rRNA_Free_Fasta.split('/')[-1].split('_200bp_Xcont.fasta')[0]+'_NorRNAseqs.fasta'
	fasta_folder = rRNA_Free_Fasta.split('_200bp_Xcont.fasta')[0]
	os.system('python3 2b_remove_Bact.py --input_file '+fasta_folder+'/'+fasta_withBact)
