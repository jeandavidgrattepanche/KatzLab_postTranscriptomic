...
def check_args():
	...
	args.file_prefix = args.input_file.split('/')[-1].split('_Filtered.')[0]
...
def usage_msg():
	...
	' --input_file ../ToRename/Op_me_Xxma_Filtered.UC.Final.AA.ORF.fasta --name Op_me_Xxma'+color.END)
...
def prep_folders(args):
	for foldername in os.listdir(os.curdir+'./'):
		if args.file_prefix in foldername:
			args.rename = foldername
			print(foldername)
...
# 	os.system('cp '+home_folder+'*_XX_*NTD.ORF.fasta ../'+args.file_prefix)
	...
	if os.path.isdir('../'+args.rename+'/Finished') != True:
		os.system('mkdir ../'+args.rename+'/Finished')
	
	print('mv ../'+args.file_prefix+'/* ../'+args.rename+'/Finished/')
	os.system('mv ../'+args.file_prefix+'/* ../'+args.rename+'/Finished/')

	os.system('rm -R ../'+args.file_prefix)
