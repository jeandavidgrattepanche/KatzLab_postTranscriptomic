...
# 		print(Folder)
		if code in Folder:
			seqfolder = os.curdir+'./'+Folder+'/Filtered_partial'	
			covupd = {}
			try:
				...
				if spreadsh.endswith('.tsv'):
					...
					for row in open(seqfolder+'/SpreadSheets/'+ spreadsh, 'r'):
						if row.split('_Trans')[0] in covupd:
							newcov2= int(covupd[row.split('_Trans')[0]] / int(row.split('_Len')[1].split('_')[0]))
	...
	# 					print(seqfile)
					...
					for Seq in SeqIO.parse(seqfolder+'/'+seqfile ,'fasta'):
						if Seq.description.split('_Trans')[0] not in covupd:
							...
							newcov= int(covupd[Seq.description.split('_Trans')[0]] / int(Seq.description.split('_Len')[1].split('_')[0]))
	# 							print(Seq.description,'\t',newcov)
			...
			os.system('mv ../'+Folder+'/Updated_Coverage/ ../'+Folder+'/Filtered_partial/')
